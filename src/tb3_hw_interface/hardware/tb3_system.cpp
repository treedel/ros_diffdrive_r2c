...
namespace tb3_hw_interface {
    ...
    hardware_interface::return_type tb3_hw_interface ::Tb3SystemHardware::write(const rclcpp::Time &, const rclcpp::Duration &) {
        ...
        if (motor_l_counts_per_sec != config_.target_counts_l || motor_r_counts_per_sec != config_.target_counts_r) {
            int abs_ml = abs(motor_l_counts_per_sec);
            int abs_mr = abs(motor_r_counts_per_sec);
            if (abs_ml>0 && abs_ml<MIN_CPS) motor_l_counts_per_sec = (motor_l_counts_per_sec>0) ? MIN_CPS : -MIN_CPS;
            if (abs_mr>0 && abs_mr<MIN_CPS) motor_r_counts_per_sec = (motor_r_counts_per_sec>0) ? MIN_CPS : -MIN_CPS;
            comms_.set_motor_values(motor_l_counts_per_sec, motor_r_counts_per_sec);
            config_.target_counts_l = motor_l_counts_per_sec;
            config_.target_counts_r = motor_r_counts_per_sec;
        }
